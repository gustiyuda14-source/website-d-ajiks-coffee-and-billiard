...
- [x] Fix bug `metadataBase` (mengarah ke domain 404, semua link-preview rusak sejak awal) → domain production benar `dajiks.vercel.app` (dikonfirmasi via `vercel project ls`) (Claude Code)
- [x] Kartu preview share (og:image) di-generate satu kali via `app/opengraph-image.tsx` (`next/og`) — logo emas + wordmark Cinzel + tagline di atas gradient brand, dipakai konsisten di semua halaman (pola sama seperti epicmoment-photobooth.vercel.app), bukan foto venue berbeda-beda per halaman (Claude Code)
